...
function main() {
  ...
  const plan = JSON.parse(readFileSync(path.join(projectRoot, 'release/m3-m4-0.10.69-candidate-plan.json'), 'utf8'));
  validateGeneratedNativeReleaseMetadata({ platform, plan, files: readCanonicalFiles(platform) });
  console.log(`Generated ${platform} native release metadata exactly matches the 0.10.69 candidate plan.`);
}
