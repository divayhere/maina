...
const historicalPlan = json('release/m3-m4-0.10.56-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.56.schema.json');
const plan = json('release/m3-m4-0.10.57-candidate-plan.json');
const schema = json('release/provenance-0.10.57.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.56-candidate-plan.json'), 'cb87cb3dbae883e123b122d382717d29c170db75db189c8120462c2737e478d1');
assert.equal(sha256('release/provenance-0.10.56.schema.json'), '96869395b72f6833aaf977f6268645f51391c41725f50c7d33a629a5226363a6');
assert.deepEqual(historicalPlan.release, { version: '0.10.56', androidVersionCode: 82, iosBuildNumber: '38' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.56');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.57');
assert.deepEqual(plan.release, { version: '0.10.57', androidVersionCode: 83, iosBuildNumber: '39' });
assert.equal(plan.sources.android.productCommit, 'c285e233b2fe3f3c422250a437a9a24db091a61f');
assert.equal(plan.sources.ios.productCommit, 'b1512d737a0b62019feab851d4df74c14fc1ef2f');
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>39<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.57.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.57-candidate-plan\.json/, `${relative} must use the active 0.10.57 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.57-83\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.57-39\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.57-39\.app\.dSYM\.zip/);
...
console.log('0.10.57 paired reliability and default-off meeting-tag candidate identity, frozen 0.10.56 evidence, defaults, exact source pins, call-interruption policy, and native terminal safety verified.');
