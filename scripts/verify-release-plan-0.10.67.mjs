...
const historicalPlan = json('release/m3-m4-0.10.66-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.66.schema.json');
const plan = json('release/m3-m4-0.10.67-candidate-plan.json');
const schema = json('release/provenance-0.10.67.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.66-candidate-plan.json'), '66ac574583b67ea21d71e521d8cf814b3901cf066734aca6de57d7dcbc204fb5');
assert.equal(sha256('release/provenance-0.10.66.schema.json'), 'bb236c2154699b2b9910a291a39c4fac7c8daa595c1e866e684844fa0581d239');
assert.deepEqual(historicalPlan.release, { version: '0.10.66', androidVersionCode: 92, iosBuildNumber: '48' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.66');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.67');
assert.deepEqual(plan.release, { version: '0.10.67', androidVersionCode: 93, iosBuildNumber: '49' });
...
expectedArtifactPolicy.android.permissions = expectedArtifactPolicy.android.permissions.filter((permission) => ![
  'android.permission.READ_EXTERNAL_STORAGE',
  'android.permission.SYSTEM_ALERT_WINDOW',
  'android.permission.WRITE_EXTERNAL_STORAGE',
].includes(permission));
const legacyCommandReceiver = expectedArtifactPolicy.android.components.find(
  (component) => component.name === 'com.divay.maina.recorder.MainaCommandReceiver',
);
assert.ok(legacyCommandReceiver);
legacyCommandReceiver.exported = 'false';
expectedArtifactPolicy.android.components.push({
  type: 'receiver',
  name: 'com.divay.maina.recorder.MainaShellCommandReceiver',
  exported: 'true',
  permission: 'android.permission.DUMP',
  process: null,
});
expectedArtifactPolicy.android.components.sort(
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.67<\/string>/);
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>49<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.67.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.67-candidate-plan\.json/, `${relative} must use the active 0.10.67 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.67-93\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.67-49\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.67-49\.app\.dSYM\.zip/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_VERSION = "0\.10\.67"/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_BUILD = "49"/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.67\.mjs/);
...
console.log('0.10.67 paired reliability and default-off meeting-tag candidate identity, frozen 0.10.66 evidence, defaults, exact source pins, call-interruption policy, iOS native stage recovery, and native terminal safety verified.');
