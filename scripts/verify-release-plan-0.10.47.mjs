...
const historicalPlan = json('release/m3-m4-0.10.46-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.46.schema.json');
const plan = json('release/m3-m4-0.10.47-candidate-plan.json');
const schema = json('release/provenance-0.10.47.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.46-candidate-plan.json'), '126306b6d99a40372bb3d4858b8a35acf1c672d787312f27cb02b7a7bfbad90e');
assert.equal(sha256('release/provenance-0.10.46.schema.json'), '396f4ce93b8fb1433f72dba726683b2a65c20863400757fd29fc7b91c9612f9c');
assert.deepEqual(historicalPlan.release, { version: '0.10.46', androidVersionCode: 72, iosBuildNumber: '28' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.46');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.47');
assert.deepEqual(plan.release, { version: '0.10.47', androidVersionCode: 73, iosBuildNumber: '29' });
assert.equal(plan.sources.android.productCommit, '0a55af059a148d54f3d842261ff70d09bd197cb0');
assert.equal(plan.sources.ios.productCommit, '170e4ee581af2912716845b7163b4cc45e4874e3');
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.47<\/string>/);
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>29<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.47.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.47-candidate-plan\.json/, `${relative} must use the active 0.10.47 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.47-73\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.47-29\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.47-29\.app\.dSYM\.zip/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.47\.mjs/);
...
console.log('0.10.47 paired call-recovery candidate identity, frozen 0.10.46 evidence plan, defaults, call-interruption truth, and drawer qualification policy verified.');
