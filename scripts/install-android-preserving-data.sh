...
[[ -n "$BUILD_TOOLS_VERSION" ]] || { echo "ANDROID_INSTALL_TOOLCHAIN_UNAVAILABLE" >&2; exit 2; }
...
[[ "$APK" == /* ]] || { echo "Android candidate path must be absolute." >&2; exit 1; }
...
if ! "$MAINA_NODE_BIN/node" "$PROJECT_DIR/scripts/release-provenance-cli.mjs" authorize android \
  "$PROJECT_DIR/release/m3-m4-0.10.51-candidate-plan.json" "$PROVENANCE" "$APK" >/dev/null 2>&1; then
...
on_exit() {
  ...
  if [[ "$lock_acquired" == "1" && "$terminal_reconciled" != "1" ]]; then
    ...
    echo "Android install outcome is not fully reconciled; the single-flight lock was retained." >&2
  fi
  if [[ -d "$RUN_DIR" ]]; then
    rm -R -- "$RUN_DIR"
  fi
...
if [[ -e "$LOCK_DIR" ]]; then
  echo "Another Android install is running or has an unknown outcome." >&2
  echo "Resume or reconcile the existing execution; do not reinvoke adb install." >&2
  exit 75
fi

[[ "$MAINA_ADB_SERIAL" == *"._adb-tls-connect._tcp" ]] || {
  echo "ANDROID_WIFI_ENDPOINT_REJECTED" >&2
  exit 1
}
device_line="$($ADB devices -l 2>/dev/null | awk -v serial="$MAINA_ADB_SERIAL" '$1 == serial && $2 == "device" {print}')"
[[ -n "$device_line" ]] || {
  echo "ANDROID_WIFI_ENDPOINT_UNAVAILABLE" >&2
  exit 1
}
actual_endpoint="$(awk '{print $1}' <<<"$device_line")"
hardware_serial="$($ADB -s "$MAINA_ADB_SERIAL" shell getprop ro.serialno 2>/dev/null | tr -d '\r')"
model="$($ADB -s "$MAINA_ADB_SERIAL" shell getprop ro.product.model 2>/dev/null | tr -d '\r')"
[[ "$hardware_serial" == "$MAINA_DEVICE_SERIAL" && "$model" == "Pixel 9 Pro" ]] || {
  echo "ANDROID_DEVICE_IDENTITY_MISMATCH" >&2
  exit 1
}
installed_package_line="$($ADB -s "$MAINA_ADB_SERIAL" shell pm list packages "$PACKAGE_NAME" 2>/dev/null | tr -d '\r')"
[[ "$installed_package_line" == "package:$PACKAGE_NAME" ]] || {
  echo "ANDROID_INSTALLED_PACKAGE_MISMATCH" >&2
  exit 1
}
...
inspect_installed() {
  ...
  installed_path="$($ADB -s "$MAINA_ADB_SERIAL" shell pm path "$PACKAGE_NAME" 2>/dev/null | head -1 | cut -d: -f2 | tr -d '\r')"
  $ADB -s "$MAINA_ADB_SERIAL" pull "$installed_path" "$output_apk" >/dev/null 2>&1
  package_dump="$($ADB -s "$MAINA_ADB_SERIAL" shell dumpsys package "$PACKAGE_NAME" 2>/dev/null | tr -d '\r')"
...
if ! "$MAINA_NODE_BIN/node" --input-type=module - "$actual_endpoint" "$MAINA_ADB_SERIAL" "$MAINA_DEVICE_SERIAL" "$hardware_serial" "$model" "$PACKAGE_NAME" "$installed_package" "$candidate_package" "$INSTALLED_SIGNER" "$candidate_signer" >/dev/null 2>&1 <<'NODE'
...
NODE
then
  echo "ANDROID_UPDATE_IDENTITY_REJECTED" >&2
  exit 1
fi
...
set +e
$ADB -s "$MAINA_ADB_SERIAL" install -r "$APK" > "$RUN_DIR/install-output.txt" 2>&1
install_status=$?
set -e
if [[ "$install_status" != "0" ]]; then
  if grep -Eq '^Failure \[[^]]+\]$' "$RUN_DIR/install-output.txt"; then
    write_lock_state "terminal_failure"
    terminal_reconciled=1
    rm -R -- "$LOCK_DIR"
    lock_acquired=0
    echo "Android package manager returned a terminal install failure." >&2
  else
    write_lock_state "reconciliation_required"
    echo "Android install outcome is ambiguous; the mutation lock was retained for explicit reconciliation." >&2
  fi
  exit "$install_status"
fi

inspect_installed "$RUN_DIR/installed-after.apk"
if ! "$MAINA_NODE_BIN/node" --input-type=module - "$candidate_sha256" "$INSTALLED_SHA256" "$candidate_signer" "$INSTALLED_SIGNER" "$candidate_version_code" "$INSTALLED_VERSION_CODE" "$candidate_version_name" "$INSTALLED_VERSION_NAME" >/dev/null 2>&1 <<'NODE'
...
NODE
then
  echo "ANDROID_INSTALL_RECONCILIATION_FAILED" >&2
  exit 1
fi
