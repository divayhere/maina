...
device_line="$($ADB devices -l | awk -v serial="$MAINA_ADB_SERIAL" '$1 == serial {print}')"
[[ "$device_line" == *"usb:"* && "$device_line" == *"device"* ]] || {
  echo "Configured Pixel is not connected over USB: $MAINA_ADB_SERIAL" >&2
  exit 1
}
...
node --input-type=module - "$MAINA_ADB_SERIAL" "$PACKAGE_NAME" "$installed_signer" "$candidate_signer" "$candidate_package" <<'NODE'
import { validateAndroidUpdate } from './scripts/lib/renewal-core.mjs';
const [, , serial, expectedPackageName, installedSigner, candidateSigner, packageName] = process.argv;
validateAndroidUpdate({ serial, expectedSerial: serial, connection: 'usb', packageName, expectedPackageName, installedSigner, candidateSigner });
