...
const historicalPlan = json('release/m3-m4-0.10.57-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.57.schema.json');
const plan = json('release/m3-m4-0.10.58-candidate-plan.json');
const schema = json('release/provenance-0.10.58.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.57-candidate-plan.json'), '384189575cb8732c5e9cead9c0feb2096d3e625035381aa745ffc315de15aba6');
assert.equal(sha256('release/provenance-0.10.57.schema.json'), 'a1c9bd900dce06d9aefec187a389a1f2e8bda51fcfd0f3cc3c65ea21e707aa8b');
assert.deepEqual(historicalPlan.release, { version: '0.10.57', androidVersionCode: 83, iosBuildNumber: '39' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.57');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.58');
assert.deepEqual(plan.release, { version: '0.10.58', androidVersionCode: 84, iosBuildNumber: '40' });
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.58.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.58-candidate-plan\.json/, `${relative} must use the active 0.10.58 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.58-84\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.58-40\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.58-40\.app\.dSYM\.zip/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_VERSION = "0\.10\.58"/);
