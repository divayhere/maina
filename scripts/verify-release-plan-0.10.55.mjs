...
const historicalPlan = json('release/m3-m4-0.10.54-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.54.schema.json');
const plan = json('release/m3-m4-0.10.55-candidate-plan.json');
const schema = json('release/provenance-0.10.55.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.54-candidate-plan.json'), '7e90859b3f5a674e176916c7a303187537819b3e275b8f2284c8a7837cf1707f');
assert.equal(sha256('release/provenance-0.10.54.schema.json'), '47d82230ec8cb85846b2fb3a7c0c1a3ff89a57d73d73247a83a0d59706137dcf');
assert.deepEqual(historicalPlan.release, { version: '0.10.54', androidVersionCode: 80, iosBuildNumber: '36' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.54');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.55');
assert.deepEqual(plan.release, { version: '0.10.55', androidVersionCode: 81, iosBuildNumber: '37' });
assert.equal(plan.sources.android.productCommit, '4015ae0632fa777b52f1f18dcc5e76ff217f2428');
assert.equal(plan.sources.ios.productCommit, 'a62548a72c0a53d56b2123447d5fb57a7840c454');
assert.equal(plan.sources.coordinationCommit, 'e5fbd0347a2d0695885a4e09a0a76c67a2a7351e');
assert.equal(
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.55<\/string>/);
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>37<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.55.schema.json',
...
assert.match(source('modules/maina-recorder/android/src/test/java/com/divay/maina/recorder/MainaCallInterruptionPolicyTest.kt'), /process death finalizes every non-manual active phase and never auto resumes/);
assert.match(source('modules/maina-recorder/android/src/test/java/com/divay/maina/recorder/MainaCallInterruptionPolicyTest.kt'), /system resume persistence failure stays latched system owned and retry eligible/);
assert.match(source('modules/maina-recorder/android/src/test/java/com/divay/maina/recorder/MainaCallInterruptionPolicyTest.kt'), /resume tap coalesces behind an in-flight native pause checkpoint/);
assert.match(source('modules/maina-recorder/android/src/test/java/com/divay/maina/recorder/MainaCallInterruptionPolicyTest.kt'), /record screen waits for native recording ownership before clearing paused UI/);
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.55-candidate-plan\.json/, `${relative} must use the active 0.10.55 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.55-81\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.55-37\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.55-37\.app\.dSYM\.zip/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.55\.mjs/);
...
console.log('0.10.55 paired exactly-once Pause/Resume candidate identity, frozen 0.10.54 evidence, defaults, exact source pins, call-interruption policy, and native terminal safety verified.');
