...
cd "$PROJECT_DIR"
node scripts/release-provenance-cli.mjs authorize ios release/m3-m4-0.10.48-candidate-plan.json "$PROVENANCE" "$APP_ZIP"
IFS=$'\t' read -r _ _ _ EXPECTED_BUNDLE_ID EXPECTED_VERSION EXPECTED_BUILD \
  <<< "$(node scripts/release-provenance-cli.mjs replay-config release/m3-m4-0.10.48-candidate-plan.json "$PROVENANCE")"
