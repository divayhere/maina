...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '70' > "$state/version-code"
    printf '0.10.44' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='70' versionName='0.10.44' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '70');
    writeFileSync(join(state, 'version-name'), '0.10.44');
  } else {
