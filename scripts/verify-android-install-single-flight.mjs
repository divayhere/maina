...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '71' > "$state/version-code"
    printf '0.10.45' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='71' versionName='0.10.45' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '71');
    writeFileSync(join(state, 'version-name'), '0.10.45');
  } else {
