...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '84' > "$state/version-code"
    printf '0.10.58' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='84' versionName='0.10.58' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '84');
    writeFileSync(join(state, 'version-name'), '0.10.58');
  } else {
