...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '91' > "$state/version-code"
    printf '0.10.65' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='91' versionName='0.10.65' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '91');
    writeFileSync(join(state, 'version-name'), '0.10.65');
  } else {
