...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '79' > "$state/version-code"
    printf '0.10.53' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='79' versionName='0.10.53' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '79');
    writeFileSync(join(state, 'version-name'), '0.10.53');
  } else {
