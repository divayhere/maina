...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '78' > "$state/version-code"
    printf '0.10.52' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='78' versionName='0.10.52' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '78');
    writeFileSync(join(state, 'version-name'), '0.10.52');
  } else {
