...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '75' > "$state/version-code"
    printf '0.10.49' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='75' versionName='0.10.49' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '75');
    writeFileSync(join(state, 'version-name'), '0.10.49');
  } else {
