...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '89' > "$state/version-code"
    printf '0.10.63' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='89' versionName='0.10.63' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '89');
    writeFileSync(join(state, 'version-name'), '0.10.63');
  } else {
