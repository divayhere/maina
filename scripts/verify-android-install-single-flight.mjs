...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '96' > "$state/version-code"
    printf '0.10.70' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='96' versionName='0.10.70' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '96');
    writeFileSync(join(state, 'version-name'), '0.10.70');
  } else {
