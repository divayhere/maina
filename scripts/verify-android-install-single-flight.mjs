...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '74' > "$state/version-code"
    printf '0.10.48' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='74' versionName='0.10.48' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '74');
    writeFileSync(join(state, 'version-name'), '0.10.48');
  } else {
