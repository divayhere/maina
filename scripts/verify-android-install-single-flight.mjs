...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  install:-r:*)
    ...
    printf '90' > "$state/version-code"
    printf '0.10.64' > "$state/version-name"
...
printf "package: name='com.divay.maina' versionCode='90' versionName='0.10.64' platformBuildVersionName=''\\n"
`);
...
function resetInstalled({ identical = false } = {}) {
  ...
  if (identical) {
    ...
    writeFileSync(join(state, 'version-code'), '90');
    writeFileSync(join(state, 'version-name'), '0.10.64');
  } else {
