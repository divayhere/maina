...
const historicalPlan = json('release/m3-m4-0.10.58-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.58.schema.json');
const plan = json('release/m3-m4-0.10.59-candidate-plan.json');
const schema = json('release/provenance-0.10.59.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.58-candidate-plan.json'), '8607391b0aee0c9c2febb61a3f97071632eddc6378255665752ecfed22981aaf');
assert.equal(sha256('release/provenance-0.10.58.schema.json'), 'a07f98fc0cfaa990b9802454b1a2b20564a41f8ad02d5225e0237a5035535c21');
assert.deepEqual(historicalPlan.release, { version: '0.10.58', androidVersionCode: 84, iosBuildNumber: '40' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.58');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.59');
assert.deepEqual(plan.release, { version: '0.10.59', androidVersionCode: 85, iosBuildNumber: '41' });
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.59<\/string>/);
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>41<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.59.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.59-candidate-plan\.json/, `${relative} must use the active 0.10.59 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.59-85\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.59-41\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.59-41\.app\.dSYM\.zip/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_VERSION = "0\.10\.59"/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_BUILD = "41"/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.59\.mjs/);
...
console.log('0.10.59 paired reliability and default-off meeting-tag candidate identity, frozen 0.10.58 evidence, defaults, exact source pins, call-interruption policy, and native terminal safety verified.');
