...
const historicalPlan = json('release/m3-m4-0.10.61-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.61.schema.json');
const plan = json('release/m3-m4-0.10.62-candidate-plan.json');
const schema = json('release/provenance-0.10.62.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.61-candidate-plan.json'), 'dedd23f3aa4b98656cd790d6b0c2e92be7f655e2376d8a86692fac67da635ce2');
assert.equal(sha256('release/provenance-0.10.61.schema.json'), '678547d66efdf5a7b8bd6b7106219b89dabaa6fc7a55a2b3cf642a5ba1b2be44');
assert.deepEqual(historicalPlan.release, { version: '0.10.61', androidVersionCode: 87, iosBuildNumber: '43' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.61');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.62');
assert.deepEqual(plan.release, { version: '0.10.62', androidVersionCode: 88, iosBuildNumber: '44' });
assert.equal(plan.sources.android.productCommit, 'b54b9bd62e4ff554cf5a73c46fc1a20e7eab806f');
assert.equal(plan.sources.ios.productCommit, '1efdb5457906fe3dcae3e20e1088e1c0874e0ca0');
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.62<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.62.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.62-candidate-plan\.json/, `${relative} must use the active 0.10.62 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.62-88\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.62-44\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.62-44\.app\.dSYM\.zip/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_VERSION = "0\.10\.62"/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_BUILD = "44"/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.62\.mjs/);
...
console.log('0.10.62 paired reliability and default-off meeting-tag candidate identity, frozen 0.10.61 evidence, defaults, exact source pins, call-interruption policy, and native terminal safety verified.');
