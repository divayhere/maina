...
const historicalPlan = json('release/m3-m4-0.10.48-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.48.schema.json');
const plan = json('release/m3-m4-0.10.49-candidate-plan.json');
const schema = json('release/provenance-0.10.49.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.48-candidate-plan.json'), '45f802169bd56a7bbd0a44cc491cb99c57e7a221a1d0bc0944f7bbf70963629a');
assert.equal(sha256('release/provenance-0.10.48.schema.json'), '9c98c0a439f45a817a65886dff44ebd9cf84c1e02886b1fbba01882e4b01aadd');
assert.deepEqual(historicalPlan.release, { version: '0.10.48', androidVersionCode: 74, iosBuildNumber: '30' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.48');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.49');
assert.deepEqual(plan.release, { version: '0.10.49', androidVersionCode: 75, iosBuildNumber: '31' });
assert.equal(plan.sources.android.productCommit, '811837b0f5d5b51c78bce8cca03cfaddd02bb609');
assert.equal(plan.sources.ios.productCommit, '9dc696af6539b6b4f526b02044b395231e834ef6');
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.49<\/string>/);
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>31<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.49.schema.json',
...
assert.match(drawer.requiredSemantics, /scrim and drawer surface containers are non-accessible/);
assert.match(drawer.requiredSemantics, /scopes modal accessibility on iOS/);
...
assert.match(source('modules/maina-recorder/ios/MainaIOSCallRecoveryPolicy.swift'), /refreshedCommunicationActive/);
assert.match(source('scripts/fixtures/MainaIOSCallRecoveryPolicyTests.swift'), /ended\/rejected call must clear stale cached call state/);
assert.match(source('scripts/verify-ios-native-recorder.mjs'), /MainaIOSCallRecoveryPolicy\.refreshedCommunicationActive/);
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.49-candidate-plan\.json/, `${relative} must use the active 0.10.49 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.49-75\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.49-31\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.49-31\.app\.dSYM\.zip/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.49\.mjs/);
...
console.log('0.10.49 paired call-recovery and drawer-accessibility candidate identity, frozen 0.10.48 evidence plan, defaults, call-interruption truth, and drawer qualification policy verified.');
