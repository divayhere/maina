...
export function collectAndroidPreflight({ env = process.env, run = runCommand } = {}) {
  ...
  const helpers = [
    ...
    'release/m3-m4-0.10.60-candidate-plan.json',
  ];
...
printf 'com.divay.maina\\t0.10.60\\t86\\tcom.divay.maina.staging\\t0.10.60\\t42\\n'
`);
...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  shell:dumpsys:package) printf 'versionName=0.10.60\\nversionCode=86 minSdk=24\\n' ;;
