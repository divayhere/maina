...
export function collectAndroidPreflight({ env = process.env, run = runCommand } = {}) {
  ...
  const helpers = [
    ...
    'release/m3-m4-0.10.55-candidate-plan.json',
  ];
...
printf 'com.divay.maina\\t0.10.55\\t81\\tcom.divay.maina.staging\\t0.10.55\\t37\\n'
`);
...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  shell:dumpsys:package) printf 'versionName=0.10.55\\nversionCode=81 minSdk=24\\n' ;;
