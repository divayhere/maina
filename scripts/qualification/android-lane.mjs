...
export function collectAndroidPreflight({ env = process.env, run = runCommand } = {}) {
  ...
  const helpers = [
    ...
    'release/m3-m4-0.10.54-candidate-plan.json',
  ];
...
printf 'com.divay.maina\\t0.10.54\\t80\\tcom.divay.maina.staging\\t0.10.54\\t36\\n'
`);
...
case "\${1:-}:\${2:-}:\${3:-}" in
  ...
  shell:dumpsys:package) printf 'versionName=0.10.54\\nversionCode=80 minSdk=24\\n' ;;
