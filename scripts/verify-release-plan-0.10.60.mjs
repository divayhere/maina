...
const historicalPlan = json('release/m3-m4-0.10.59-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.59.schema.json');
const plan = json('release/m3-m4-0.10.60-candidate-plan.json');
const schema = json('release/provenance-0.10.60.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.59-candidate-plan.json'), '6cd0c09e16981fb8db2fdae3e0f38430a641e40809e728c21800697ae1eaf08e');
assert.equal(sha256('release/provenance-0.10.59.schema.json'), '312d49822043b3abafc72c43d33cbc9cf032bca7395660dcdefebc9c90b957bf');
assert.deepEqual(historicalPlan.release, { version: '0.10.59', androidVersionCode: 85, iosBuildNumber: '41' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.59');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.60');
assert.deepEqual(plan.release, { version: '0.10.60', androidVersionCode: 86, iosBuildNumber: '42' });
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.60<\/string>/);
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>42<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.60.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.60-candidate-plan\.json/, `${relative} must use the active 0.10.60 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.60-86\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.60-42\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.60-42\.app\.dSYM\.zip/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_VERSION = "0\.10\.60"/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_BUILD = "42"/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.60\.mjs/);
...
console.log('0.10.60 paired reliability and default-off meeting-tag candidate identity, frozen 0.10.59 evidence, defaults, exact source pins, call-interruption policy, and native terminal safety verified.');
