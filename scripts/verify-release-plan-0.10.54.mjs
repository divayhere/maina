...
const historicalPlan = json('release/m3-m4-0.10.53-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.53.schema.json');
const plan = json('release/m3-m4-0.10.54-candidate-plan.json');
const schema = json('release/provenance-0.10.54.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.53-candidate-plan.json'), '5b11245b953b0ea229cecd86acd783345b89b6851bc2515d4b74d87531c46b9c');
assert.equal(sha256('release/provenance-0.10.53.schema.json'), 'f73092fbf4ebeded5b1f2bcdaf6cb2c8a2e0c94cfd08c9e8ba67adc46b31937c');
assert.deepEqual(historicalPlan.release, { version: '0.10.53', androidVersionCode: 79, iosBuildNumber: '35' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.53');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.54');
...
assert.equal(plan.sources.coordinationCommit, '3f3ed48d00dae7fefcdf149a74816a954a048385');
assert.equal(
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.54<\/string>/);
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>36<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.54.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.54-candidate-plan\.json/, `${relative} must use the active 0.10.54 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.54-80\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.54-36\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.54-36\.app\.dSYM\.zip/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.54\.mjs/);
...
console.log('0.10.54 paired manual-resume ownership candidate identity, frozen 0.10.53 evidence, defaults, exact source pins, call-interruption policy, and native terminal safety verified.');
