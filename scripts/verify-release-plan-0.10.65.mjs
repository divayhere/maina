...
const historicalPlan = json('release/m3-m4-0.10.64-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.64.schema.json');
const plan = json('release/m3-m4-0.10.65-candidate-plan.json');
const schema = json('release/provenance-0.10.65.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.64-candidate-plan.json'), 'f61680d82dc2e510dc14b45ad59498a934d3555687f3da50220a045d3d874c7b');
assert.equal(sha256('release/provenance-0.10.64.schema.json'), '116d47d7616c3a07123068a6f8897b444907e87e68423d2031e53dc47391f4ef');
assert.deepEqual(historicalPlan.release, { version: '0.10.64', androidVersionCode: 90, iosBuildNumber: '46' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.64');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.65');
assert.deepEqual(plan.release, { version: '0.10.65', androidVersionCode: 91, iosBuildNumber: '47' });
assert.equal(plan.sources.android.productCommit, '914b5cd501d0789f3c02dbf9f9970543d99787bf');
assert.equal(plan.sources.ios.productCommit, '725c547a4d3b626f36aef399f44ac1adb1533387');
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.65<\/string>/);
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>47<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.65.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.65-candidate-plan\.json/, `${relative} must use the active 0.10.65 plan.`);
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.65-91\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.65-47\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.65-47\.app\.dSYM\.zip/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_VERSION = "0\.10\.65"/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_BUILD = "47"/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.65\.mjs/);
...
console.log('0.10.65 paired reliability and default-off meeting-tag candidate identity, frozen 0.10.64 evidence, defaults, exact source pins, call-interruption policy, iOS native stage recovery, and native terminal safety verified.');
