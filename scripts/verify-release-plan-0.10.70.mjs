...
const historicalPlan = json('release/m3-m4-0.10.69-candidate-plan.json');
const historicalSchema = json('release/provenance-0.10.69.schema.json');
const plan = json('release/m3-m4-0.10.70-candidate-plan.json');
const schema = json('release/provenance-0.10.70.schema.json');
const app = json('app.json').expo;
const manifest = json('package.json');
const lock = json('package-lock.json');

assert.equal(sha256('release/m3-m4-0.10.69-candidate-plan.json'), '3d31ad64c77d137b2e1fefe3b421dedca2c1b1590ee2df3225b7ef988d0e2e4b');
assert.equal(sha256('release/provenance-0.10.69.schema.json'), '4d5418eb26dd946361b0dc2c5b28857b4985c4339627551b9dd7c45d77a768a5');
assert.deepEqual(historicalPlan.release, { version: '0.10.69', androidVersionCode: 95, iosBuildNumber: '51' });
assert.equal(historicalSchema.properties.releaseId.const, 'maina-m3-m4-0.10.69');

assert.equal(plan.releaseId, 'maina-m3-m4-0.10.70');
assert.deepEqual(plan.release, { version: '0.10.70', androidVersionCode: 96, iosBuildNumber: '52' });
assert.equal(plan.sources.android.productCommit, '384acd0e325dd3e523fefe7e935c726cca36c4d9');
assert.equal(plan.sources.ios.productCommit, '7c43d9f69992c46bfc8a267794e674498ef8362e');
...
assert.notDeepEqual(schema, historicalSchema, '0.10.70 must retain its recursively closed provenance schema hardening.');
...
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.10\.70<\/string>/);
assert.match(source('ios/Maina/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>52<\/string>/);
...
assert.deepEqual(plan.provenancePolicy, {
  schema: 'release/provenance-0.10.70.schema.json',
...
for (const relative of [
  ...
  assert.match(source(relative), /m3-m4-0\.10\.70-candidate-plan\.json/, `${relative} must use the active 0.10.70 plan.`);
  assert.doesNotMatch(source(relative), /m3-m4-0\.10\.69-candidate-plan\.json/, `${relative} must not use the frozen 0.10.69 plan.`);
}
function assertIosLaneActivePlanBinding(value) {
  assert.equal(
    (value.match(/const activeReleasePlanRelativePath = 'release\/m3-m4-0\.10\.70-candidate-plan\.json';/g) ?? []).length,
    1,
    'iOS qualification must declare exactly one active 0.10.70 plan authority.',
  );
  assert.equal(
...
for (const staleVersion of ['0.10.69', '0.10.68', '0.10.67']) {
  assert.throws(
    () => assertIosLaneActivePlanBinding(iosLaneSource.replace('0.10.70-candidate-plan.json', `${staleVersion}-candidate-plan.json`)),
    /active 0\.10\.70 plan authority/,
  );
}
assert.match(source('scripts/build-android-release-candidate.sh'), /Maina-0\.10\.70-96\.apk/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.70-52\.app\.zip/);
assert.match(source('scripts/build-ios-release-candidate.sh'), /Maina-0\.10\.70-52\.app\.dSYM\.zip/);
...
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_VERSION = "0\.10\.70"/);
assert.match(source('scripts/run-ios-xcuitest-direct.py'), /EXPECTED_BUILD = "52"/);
...
assert.match(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.70\.mjs/);
assert.doesNotMatch(manifest.scripts['verify:release-plan-candidate'], /verify-release-plan-0\.10\.69\.mjs/);
...
console.log('0.10.70 paired reliability and recording database-admission candidate identity, frozen 0.10.69 evidence, defaults, exact source pins, call-interruption policy, iOS native stage recovery, and native terminal safety verified.');
