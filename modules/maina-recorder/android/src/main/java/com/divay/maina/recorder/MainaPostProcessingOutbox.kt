...
internal class MainaPostProcessingOutbox(context: Context) :
    ...
    fun markWindow(
        ...
        ContentValues().apply {
            ...
            if (evidence == null) {
                ...
                putNull("token_count")
                putNull("truncation_suspected")
                putNull("retry_reason")
                putNull("thermal_status")
                ...
                put("truncation_suspected", if (evidence.truncationSuspected) 1 else 0)
                if (evidence.retryReason == null) putNull("retry_reason") else put("retry_reason", evidence.retryReason)
                if (evidence.thermalStatus == null) putNull("thermal_status") else put("thermal_status", evidence.thermalStatus)
    ...
    private fun addWindowEvidenceColumns(db: SQLiteDatabase) {
        listOf(
            "processing_ms INTEGER",
            "token_count INTEGER",
            "truncation_suspected INTEGER",
